...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($_SERVER['CONTENT_TYPE'], 'application/json') !== false) {
    ...
    if (isset($data['bookingId'])) {
        ...
        $checkSql = "SELECT BookingID, BookingDate FROM booking WHERE BookingID = ?";
        ...
        if ($stmt) {
            ...
            if ($result->num_rows > 0) {
                // ส่งผู้ใช้งานไปยังหน้า Detailbooking.php เพื่อเลือกสนามและอัปเดตข้อมูล
                echo json_encode([
                    "success" => true, 
                    "message" => "โปรดเลือกสนามก่อนเปลี่ยนสถานะ", 
                    "redirect" => "../admin/Detailbooking.php?bookingId=" . $bookingId . "&date=" . $result->fetch_assoc()["BookingDate"]
                ]);
            } else {
