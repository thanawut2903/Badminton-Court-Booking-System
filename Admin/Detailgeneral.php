...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['accountid'])) {
    ...
    $sql = "SELECT b.BookingID, b.BookingDate, b.StartTime, b.EndTime, b.CourtID, b.Status, b.CancelReason, 
    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            $firstnamemember = $row["FirstName"];
            $lastnamemember = $row["LastName"];
        } else {
            $firstnamemember = "ไม่พบข้อมูล";
            $lastnamemember = "";
        }
    ...
    echo "ไม่มีข้อมูล AccountID ที่ถูกต้อง";
}
...
<head>
  ...
  <!-- ลิงก์ไปยังไฟล์ CSS -->
  <link href="Detailgeneral.css" rel="stylesheet">
</head>
<body>
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['accountid'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $displayEndTime = ($endTime === '00:00') ? '24:00' : $endTime;
            echo '<td>' . htmlspecialchars($startTime) . ' น.'.' - '. htmlspecialchars($displayEndTime) . ' น.'.'</td>';
